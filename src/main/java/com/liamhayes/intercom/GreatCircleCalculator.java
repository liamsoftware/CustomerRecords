...
public class GreatCircleCalculator {
    ...
    /**
     * Calculates the great circle distance in KM of the provided location to that of the Intercom Dublin office 
     * Uses the Haversine formula detailed on https://en.wikipedia.org/wiki/Great-circle_distance
     * @param customerLatitude
     * @param customerLongitude
     * @return the distance in km of the provided coordinates from the Intercom Dublin office
     * @throws Exception if the provided coordinates are out of bounds for latitude or longitude
     */
    public static double calculateDistanceInKm(double customerLatitude,
                                               double customerLongitude) throws Exception {
    	if (!isValidLatitude(customerLatitude) || !isValidLongitude(customerLongitude)) {
    		throw new Exception("Invalid Coordinates provided. LAT[" + 
    				customerLatitude + "], LON[" + customerLongitude + "]"); 
    	} 
    ...
    private static double performHaversineCalculation(double customerLatitude, 
         ...
         double a =
                 Math.pow(Math.sin(latitudeDeltaRadian / 2), 2)
                         + Math.cos(intercomLatitudeRadian)
                         * Math.cos(customerLatitudeRadian)
                         * Math.pow(Math.sin(longitudeDeltaRadian / 2), 2);
